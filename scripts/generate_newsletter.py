...
MAX_ARTICLES = int(os.getenv("NEWSLETTER_MAX_ARTICLES", "3"))
...
def inline_css(html_content: str) -> str:
    """
    Inline newsletter.css directly into each tag's `style` attribute.

    WHY: Gmail (and most other email clients) strip <link rel="stylesheet">
    tags entirely for security/privacy reasons — they never fetch external
    CSS. Since the template links to newsletter.css instead of embedding
    styles, the newsletter was rendering as unstyled plain text in Gmail.

    This keeps templates/newsletter.css as the single readable source of
    truth for styling, but bakes it into each element's `style=""`
    attribute in the generated/sent HTML so it survives being stripped.
    @media queries can't be inlined (they're inherently per-breakpoint), so
    those are kept in a <style> block for clients that do honor it (Gmail's
    mobile app, Apple Mail, etc.) — this only degrades to a fixed-width
    layout on the few clients that respect neither, it never loses colors/
    fonts/spacing the way the pure <link> approach did everywhere.
    """
    css_path = TEMPLATE_DIR / "newsletter.css"
    if not css_path.exists():
        logger.warning("⚠️  newsletter.css not found — sending without inlined styles.")
        return html_content

    css_text = css_path.read_text(encoding="utf-8")

    # Strip the <link rel="stylesheet"> tag itself — Premailer would
    # otherwise try to fetch it as a URL, and it's now redundant since the
    # same CSS is passed in directly via css_text below.
    html_no_link = re.sub(r'<link[^>]+newsletter\.css[^>]*>\s*', '', html_content)

    try:
        return Premailer(
            html=html_no_link,
            css_text=css_text,
            keep_style_tags=True,       # preserve @media rules for clients that support them
            remove_classes=False,
            strip_important=False,
            cssutils_logging_level="CRITICAL",
        ).transform()
    except Exception as e:
        logger.error(f"💥 CSS inlining failed — sending with un-inlined styles: {e}")
        return html_content
...
def run_generation_pipeline() -> Optional[Tuple[Path, Path, Path]]:
    ...
    if not articles:
        logger.warning(
            "⚠️ No summarized articles available for rendering — the 'Top Stories' "
            "section will not appear in this newsletter. This means the raw "
            "fetch/clean/summarize stages produced zero usable articles; check "
            "fetch_news.log and clean_news.log for the cause (dead feed URLs, "
            "quality filter, etc.)."
        )
