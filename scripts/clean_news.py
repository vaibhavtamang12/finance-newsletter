...
# Was 50 — several working feeds (e.g. Yahoo Finance's headline index) supply
# short or title-only descriptions, so 50 was filtering out most/all articles
# on many runs and leaving "Top Stories" empty. 15 is a safer floor: enough to
# reject truly blank/junk entries without discarding legitimate short blurbs.
...
def run_cleaning_pipeline() -> Optional[Path]:
    ...
    if not valid_articles:
        logger.warning(
            f"⚠️ Quality filter removed ALL {len(articles)} fetched articles — "
            "Top Stories will be empty in the generated newsletter. Check "
            "MIN_SUMMARY_LENGTH / CLICKBAIT_PATTERNS or the source feeds."
        )
