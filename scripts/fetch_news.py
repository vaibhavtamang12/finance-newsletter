...
#
# NOTE: Reuters killed its public RSS feeds (feeds.reuters.com) back in June 2020 —
# that URL now 404s permanently, so it was pulled from this list. It was silently
# returning zero articles every run, which (combined with the strict quality filter
# in clean_news.py) was a major reason "Top Stories" kept coming up empty.
...
def fetch_single_feed(feed_config: Dict[str, str], timeout: int = 15) -> List[Dict]:
    ...
    try:
        ...
        for entry in feed.entries:
            ...
            # Some feeds (notably headline-index feeds like Yahoo's rssindex) only
            # provide a title with no <description>. Without this fallback those
            # entries get silently dropped by clean_news.py's MIN_SUMMARY_LENGTH
            # filter, which is how "Top Stories" ends up empty even when the feed
